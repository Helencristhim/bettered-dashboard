...
// Busca o nome da primeira aba da planilha
...
export async function getData() {
  try {
    ...
    throw new Error('Erro ao acessar planilha. Verifique se a planilha foi compartilhada com a service account.')
  }
}
...
export async function findRowIndex(mes, ano, fonte) {
  try {
    ...
    console.error('Erro ao buscar indice:', error)
    return null
  }
}
...
export async function saveData(data) {
  try {
    ...
    // Verifica se ja existe registro para este mes/ano/fonte
    ...
    if (error.message?.includes('403') || error.message?.includes('permission')) {
      throw new Error('Sem permissao. Compartilhe a planilha com: ' + process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL)
    }
